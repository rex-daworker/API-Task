...
const createNote = async (req, res, next) => {
    try {
        const applicationId = req.params.id; // FIXED
        const { note, rating } = req.body;
        const app = await application_model_1.Application.findById(applicationId);
        if (!app)
            return res.status(404).json({ message: "Application not found" });
        const interviewNote = await interviewNote_model_1.InterviewNote.create({
...
const getNotes = async (req, res, next) => {
    try {
        const applicationId = req.params.id; // FIXED
        const notes = await interviewNote_model_1.InterviewNote.find({ application: applicationId })
            .sort({ created_at: -1 });
